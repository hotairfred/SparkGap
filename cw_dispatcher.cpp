                    ...
                    int n_samples)
{
    ...
    // Serial for bmorse channels. The PFB + shift + decimate + FIR still
    // all run in C++ (GIL-free), but bmorse_feed itself is single-threaded.
    for (channel_slot_t *s : live_bmorse) {
        const std::complex<float> *bin_row =
            out_c + (size_t)s->bin_idx * (size_t)out_n_steps;
