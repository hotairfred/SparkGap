...
void process_stdin()
{
	// Read 16-bit mono PCM from stdin, process continuously
	// Uses the non-FFT path (same as running without -fft flag)
	// which calls process_data() directly after envelope detection
	int sr = (int)params.sample_rate;
	params.dec_ratio = sr / BAYES_RATE;
	params.print_text = TRUE;
	params.agc = TRUE;

	fprintf(stderr, "bmorse streaming: rate=%d freq=%.0f speed=%d dec_ratio=%d agc=%d\n",
			sr, params.frequency, params.speed, params.dec_ratio, params.agc);

	// Use FFT filter path for baseband conversion + lowpass
	int FilterFFTLen = 4096;
	FFT_filter = new fftfilt((params.speed)/(1.2 * params.sample_rate), FilterFFTLen);
	...
	while (1) {
		size_t n = fread(pcm_buf, sizeof(int16_t), 512, stdin);
		if (n == 0) break;

		// Convert 16-bit PCM to double [-1.0, 1.0]
		for (size_t i = 0; i < n; i++)
			dbl_buf[i] = (double)pcm_buf[i] / 32768.0;

		rx_FFTprocess(dbl_buf, (int)n);
		fflush(stdout);
	}

	fprintf(stderr, "bmorse: stdin EOF\n");
}
...
int main(int argc, const char* argv[])
{
	...
	for (k = 1 ; k < argc; k++) {
		...
		if (strcmp (argv [k], "-stdin") == 0){
			params.process_stdin = TRUE;
			continue ;
			}
		if (strcmp (argv [k], "-rate") == 0){
			k++;
			params.sample_rate = atof(argv[k]);
			continue ;
			}
		} ;
...
/* 	INITIALIZE DATA STRUCTURES */
	...
	if (params.process_stdin)
		process_stdin();
	else if(params.process_textfile)
