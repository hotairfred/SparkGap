#!/usr/bin/env python3
"""Score UHSDR + ML + bmorse(WPM-seeded) + HamFist(WPM-seeded) quad ensemble on CWT."""
...
for si, (freq_hz, snr) in enumerate(clustered):
    ...
    # ML inference (short window) — WPM estimate + CALL extraction
    audio_4k = np.frombuffer(pcm_4k, dtype=np.int16).astype(np.float32) / 32767.0
    wpm, ml_found = ml_infer(audio_4k)
    for c in ml_found:
        if c not in ml_calls: ml_calls[c] = rf_khz

    # bmorse — seeded at ML-estimated WPM
    ...
    if (si+1) % 20 == 0:
        ...
        print(f"  Signal {si+1}/{len(clustered)}: {rf_khz:.1f} kHz — U={u} M={m} B={b} H={h} quad={quad} (last wpm={wpm})", flush=True)
...
print(f"bmorse:   {len(b_hits)}/118  (WPM-seeded)")
