...
// ONE_SECOND and CW_ONE_BIT_SAMPLE_COUNT use per-instance sample_rate (not global snd_rate)
#define ONE_SECOND			(cw->sample_rate_hz / cw->blocksize)
#define CW_ONE_BIT_SAMPLE_COUNT (ONE_SECOND / 58.3)
...
#define CW_SPIKECANCEL_MODE_OFF 0
    ...
    int outbuf_len;                // Current position in outbuf
};

// For library mode, instances are heap-allocated via uhsdr_init().
// Keep global array for backward compatibility with standalone main().
...
{
	fprintf(stderr, "AudioFilter_CalcGoertzel f=%f sr=%f\n", freq, samplerate);
...
void CwDecode_Init(int rx_chan, int wpm, int training_interval)
{
    ...
    fprintf(stderr, "CwDecode_Init wpm=%d %s\n", wpm, wpm? "FIXED":"AUTO");
    memset(cw, 0, sizeof(cw_decoder_t));
    cw->rx_chan = rx_chan;
    cw->wsc = 1;
    cw->old_siglevel = 0.001;
    cw->timer_stepsize = 1;
    cw->sampling_freq = snd_rate;  // set from global during init, per-instance after copy
    ...
    if (wpm != 0) {
        ...
        fprintf(stderr, "CW WPM FIXED INIT %.0f|%.1f spdcalc %.3f MSPW %.3f pulse %.1f dot %.1f dash %.1f space %.1f sym %.1f wsc %d threshold %s %d (%.1f dB)\n",
...
static void CW_Decode_exe(cw_decoder_t *cw)
{
	...
	if (cw->b.initialized == true && spdcalc > 0)
	{
		...
		cw->speed_wpm_avg = speed_wpm_raw * 0.3 + 0.7 * cw->speed_wpm_avg; // a little lowpass filtering
            ...
            static int cnt;
            if ((cnt & 0xff) == 0) {
                fprintf(stderr, "CW WPM %s %.0f|%.1f spdcalc %.3f MSPW %.3f pulse %.1f dot %.1f dash %.1f space %.1f sym %.1f %s %d (%.1f dB)"
                    " %.0f %.0f|(%.0f,%.0f)|%.0f\n",
	...
	{
	    //fprintf(stderr, "b.init=%d (spd>0)=%d\n", cw->b.initialized, spdcalc > 0);
...
static void cw_train(cw_decoder_t *cw)
{
	...
	if (processed >= cw->training_interval)
	{
		...
		#if 0
        fprintf(stderr, "pulse %.1f dot %.1f dash %.1f space %.1f sym %.1f\n",
            cw->times.pulse_avg, cw->times.dot_avg, cw->times.dash_avg, cw->times.cwspace_avg, cw->times.symspace_avg);
	    #endif
	}
...
static void cw_print(cw_decoder_t *cw, const char *s)
{
    ...
    for (const char *p = s; *p; p++) {
        if (cw->outbuf_len < (int)sizeof(cw->outbuf) - 1) {
            cw->outbuf[cw->outbuf_len++] = *p;
        }
    }
    // Also call global callback if set (standalone mode)
    if (cw_output_callback) {
        for (const char *p = s; *p; p++) cw_output_callback(*p, cw->speed);
    }
}
...
void CwDecode_threshold(int rx_chan, int type, int param)
{
    ...
    if (type == 0) {
        ...
        fprintf(stderr, "CW (type=1) threshold_linear=%d (%.1f dB)\n", param, 10.0 * log10f((float) param + 1e-30F));
    }
}
...
static bool ErrorCorrectionFunc(cw_decoder_t *cw)
{
	...
	else
	{
		...
		if ((pduration < cw->times.dot_avg / 2) && (plocation != temp_outcount))
		{
			...
			if (i == 1024) fprintf(stderr, "CW LOOP T/O #1\n");
		...
		else
		{
			...
			if (i == 1024) fprintf(stderr, "CW LOOP T/O #2\n");
			...
			if (i == 1024) fprintf(stderr, "CW LOOP T/O #3\n");
...
// Globals for standalone mode (kept for backward compat)
float snd_rate = 12000.0f;
cw_output_fn cw_output_callback = NULL;
...
{
    // Use the global array slot 0 for initialization (reuses CwDecode_Init),
    // then copy to heap. This ensures perfect parity with standalone.
    snd_rate = sample_rate;
    CwDecode_Init(0, wpm, 100);
    CwDecode_pboff(0, (u4_t)freq);

    cw_decoder_t *src = &cw_decoder[0];
    src->sample_rate_hz = sample_rate;
    src->process_samples = true;
    ...
    if (!cw) return NULL;
    memcpy(cw, src, sizeof(cw_decoder_t));
    cw->outbuf_len = 0;
               ...
               char *out, int outlen)
{
    ...
    // Reset output buffer for this call
    cw->outbuf_len = 0;

    // Feed samples one at a time (matches standalone behavior)
    for (int i = 0; i < n; i++) {
        int16_t s = samples[i];
        // Inline CwDecode_RxProcessor for this instance
        if (!cw->process_samples) continue;

        cw->raw_signal_buffer[cw->sample_counter] = (float32_t)s / 4;
        cw->sample_counter++;
    ...
    // Copy accumulated output to caller's buffer
    int ncopy = cw->outbuf_len;
    if (ncopy > outlen - 1) ncopy = outlen - 1;
    if (ncopy > 0) {
        memcpy(out, cw->outbuf, ncopy);
        out[ncopy] = '\0';
    }
    return ncopy;
}
...
// ============================================================================
// Standalone main (compile with -DUHSDR_STANDALONE to include)
// ============================================================================
#ifdef UHSDR_STANDALONE
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-f") == 0 && i+1 < argc) freq = atof(argv[++i]);
        else if (strcmp(argv[i], "-s") == 0 && i+1 < argc) wpm = atoi(argv[++i]);
        else if (strcmp(argv[i], "-r") == 0 && i+1 < argc) snd_rate = atof(argv[++i]);
        else if (strcmp(argv[i], "-h") == 0) {
            fprintf(stderr, "uhsdr_cw — UHSDR CW decoder (TF3LJ, GPL-3)\n");
            fprintf(stderr, "Reads 16-bit signed mono from stdin\n");
            fprintf(stderr, "  -r rate   Sample rate (default 12000)\n");
            fprintf(stderr, "  -f freq   CW tone frequency (default 600)\n");
            fprintf(stderr, "  -s wpm    Fixed speed (0=auto, default auto)\n");
            return 0;
        }
    }
    ...
    cw->sample_rate_hz = snd_rate;
    cw->process_samples = true;
    cw->isAutoThreshold = true;
    cw->weight_linear = AUTO_WEIGHT_LINEAR;
    cw->threshold_linear = 500;

    int16_t sample;
    int count = 0;
    while (fread(&sample, sizeof(int16_t), 1, stdin) == 1) {
        CwDecode_RxProcessor(0, 0, 1, &sample);
        count++;
    }

    fprintf(stderr, "Processed %d samples (%.1f s)\n", count, (float)count / snd_rate);
    return 0;
}
#endif
